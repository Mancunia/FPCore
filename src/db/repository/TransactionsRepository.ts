import Transaction,{TransactionIn,TransactionOut,TransactionUpdate} from "../models/TransactionsModel";
...
interface TransactionRepositoryInterface{
    ...
    updateTransaction:(id:number,payload:TransactionIn)=>Promise<TransactionOut>;
...
class TransactionRepository implements TransactionRepositoryInterface{
    ...
    async createTransaction(payload:TransactionIn):Promise<TransactionOut>{
        try {
            let transaction = await Transaction.create(payload)//create a new transaction
            return transaction
    ...
    async getTransaction(id:number|string):Promise<TransactionOut>{
        try{
            let transaction = await Transaction.findOne({where:{[Op.or]:
                [
                {id:id},
                {SessionID:id},
                {ReferenceID:id}
    ...
    //---------------------------------------------------------------- get Transaction ----------------------------------------------------
    async updateTransaction(id:number|string,payload:TransactionUpdate):Promise<TransactionOut>{
        try {
